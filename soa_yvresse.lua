-- Script by Xoudad (Yeah, I'm also surprised)
-- Yvresse stuff
...
core:add_listener(
	"Xoudad_ArtSetConversion",
	"CharacterSkillPointAllocated",
	function(context)
		return context:skill_point_spent_on():find("wh2_main_skill_hef_eltharion_blind")
	end,
	function(context)
		local cqi = context:character():cqi()
		for skill, artSet in pairs(skillz) do
			if context:skill_point_spent_on() == skill then
				cm:add_unit_model_overrides("character_cqi:"..cqi, artSet)
				local elthi_faction = context:character():faction()
				cm:force_remove_ancillary_from_faction(elthi_faction, "wh2_dlc15_anc_armour_helm_of_yvresse")
				cm:force_remove_ancillary_from_faction(elthi_faction, "wh2_dlc15_anc_talisman_talisman_of_hoeth")
				cm:force_remove_ancillary_from_faction(elthi_faction, "wh2_dlc15_anc_weapon_fangsword_of_eltharion")
			end
		end
	end,
	true
